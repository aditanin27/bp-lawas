import { Text, View, TouchableOpacity, StyleSheet, FlatList, RefreshControl, TextInput } from 'react-native'
import React, { Component } from 'react'
import { LocationsH, Orangbanyakhitam, FilterdanText, IconCari } from '../../assets/icons'
export class Jumlahlistshelter extends Component {
    constructor(props) {
        super(props);
        this.state = {
            laporan: [],
    ...
    GetAnakAPi() {
        fetch('https://kilauindonesia.org/datakilau/api/getlaporananak/' + this.state.laporan.id_shelter).then(res => {
            if (res.status === 200)
                return res.json()
        }).then((resdata) => {
            console.log('ini resdata get', resdata);
    ...
    render() {
        ...
        // const filtershelter =
        //     this.state.laporan.filter((item, index) => {
        //         return this.state.laporan.findIndex(i => i.nama_shelter === item.nama_shelter) === index;
        //     })
        // console.log(filtershelter)
        return (
                ...
                <FlatList
                    ...
                    renderItem={({ item, index }) => (
                        <View>
                            ...
                            <TouchableOpacity onPress={() => this.props.navigation.navigate('Detailjumlahanak', { detail: item })}>
...
export default Jumlahlistshelter