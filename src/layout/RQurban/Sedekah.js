import {
  Text, View, StyleSheet, Dimensions, Image, ScrollViewBase,
  ScrollView, TouchableOpacity, ImageBackground, Modal, TextInput
} from 'react-native'
...
class Sedekah extends Component {
  constructor(props) {
    super(props)
    this.state = {
  ...
  facebook = () => {
    let url =
      'https://www.facebook.com/sharer/sharer.php?u=https://657e147121e7.ngrok.io/program/sedekahdaging'
    Linking.openURL(url)
      .then(data => {
        console.log("Opened successfully " + data);
      })
  ...
  render() {
    ...
    // for (let i = 0; i < this.state.harga; i++) {
    //   tambah.push(
    //     <><View key={i}>
    //       <Text value={this.state.tambah[i]}>{harga}</Text>
    //     </View></>
    //   );
    // }
      ...
      <ScrollView>
        <View>
          <View style={{ backgroundColor: '#0EBEDF' }}>
            <Text style={style.title}>Sedekah Daging</Text>
          </View>
          <Image source={qurban} style={{ width: '100%', height: 170 }}></Image>
          <View style={style.coltom}>
            <View>
              <View>
                <Text style={{ marginLeft: 10, fontWeight: 'bold', fontSize: 16 }}>Harga Per Orang</Text>
                <Text style={{ marginLeft: 10, fontWeight: 'bold' }}>Rp.100.000</Text>
            ...
            <View style={{ flexDirection: 'row', justifyContent: 'flex-end', alignContent: 'center', alignItems: 'center' }}>
              <TouchableOpacity style={{ marginTop: 20, marginRight: 10 }} onPress={() => this.setState({ modalpembayaran: true })}>
                <Text style={style.btnSimpanDark}>Sedekah Sekarang</Text>
              </TouchableOpacity>
            ...
            <View>
              ...
              <View style={{ height: '80%', width: '100%', backgroundColor: 'white', borderRadius: 5, marginTop: 5 }}>
                      ...
                      <TouchableOpacity
                        ...
                        onPress={() => {
                          if (this.state.count > 0) {
                            ...
                            this.setState({ harga: this.state.harga - 100000 })
                          } else {
                ...
                <View style={{ flexDirection: 'row', justifyContent: 'center', alignContent: 'center' }}>
                  ...
                  <TouchableOpacity 
                  // onPress={() => this.setState({ pilihpembayaran: true })}
                  onPress={() => {
                    if (this.state.text.length === 0) {
                      this.setState(() => ({ nameError: " Masukan Nama." }));
                    } else {
                      this.setState(() => ({ nameError: null }));
                      this.setState({ pilihpembayaran: true })
                    }
                  }}
