...
import { Text, View, Modal, FlatList, SafeAreaView, StyleSheet, TouchableOpacity, RefreshControl, TextInput, ToastAndroid } from 'react-native'
...
export class Barang extends Component {
    ...
    filterList(textToSearch) {
        this.setState({
            filter: this.state.Barang
                .filter(item => item.nama_barang.toLowerCase(textToSearch).includes(textToSearch))
        })
    }
    Hapusbarang(detbar) {
        {
            fetch('https://kilauindonesia.org/datakilau/api/baranghps/' + detbar.id_barang, {
                method: 'POST',
                headers: {
    ...
    sendData() {
        {
            let simpandata = {
                nama_barang: this.state.baru,
            }
            let data = new FormData();
            for (let key in simpandata) {
                data.append(key, simpandata[key]);
            }
            fetch('https://kilauindonesia.org/datakilau/api/tambahbarang', {
                method: 'POST',
                headers: {
                    Accept: 'application/json',
                    'Content-Type': 'multipart/form-data'
    ...
    render() {
        const debar = this.state.debar
        const detbar = this.state.detbar
        return (
            <View style={{ backgroundColor: '#fff', flex: 1 }}>
                ...
                <FlatList
                    ...
                    renderItem={({ item, index }) => (
                        <TouchableOpacity style={style.itemflat} onPress={() => this.setState({ modalpilih: true, detbar: item })}>
                            <Text style={style.labelkiri}>{item.nama_barang}</Text>
                        </TouchableOpacity>
                    )}></FlatList>
                ...
                <View style={{ position: 'absolute', justifyContent: 'center', alignContent: 'center', alignItems: 'center', alignSelf: 'center', bottom: 10, }}>
                    <TouchableOpacity onPress={() => this.setState({ modaltambah: true })} style={style.refresh} >
                        <View style={{
                            ...
                            <Text style={{ color: '#fff', textAlign: 'center', justifyContent: 'center', alignContent: 'center' }}>Tambah Data Barang</Text>
                        </View>
                    </TouchableOpacity>
                </View>
                <Modal
                    ...
                    <SafeAreaView style={style.containerSafe}>
                        <TouchableOpacity activeOpacity={1.0} onPress={() => this.setState({ modalpilih: false })} style={style.ModalCont}>
                            ...
                            <View style={{
                                ...
                                <View style={{ flexDirection: 'row', justifyContent: 'space-between', }}>
                                    <TouchableOpacity style={[style.kotakkecil, { backgroundColor: '#B22222' }]} onPress={() => this.setState({ modalpilih: false }, this.Hapusbarang(detbar))}>
                                        <View >
                                            {/* <KKada style={{ justifyContent: 'center', alignSelf: 'center' }} /> */}
                                    ...
                                    <TouchableOpacity style={[style.kotakkecil, { backgroundColor: '#00A9B8', }]} onPress={() => this.setState({ modalpilih: false, modaledit: true, debar: this.state.debar })}>
                                        <View >
                                            {/* <KKada style={{ justifyContent: 'center', alignSelf: 'center' }} /> */}
